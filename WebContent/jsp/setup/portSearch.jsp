	...
	<t:saveState value="#{portControl.ports}"/>
			<h:form id="portsForm">
				<table width="99%">		
					<tr>
						<td>
							<rich:panel headerClass="stylePanel" bodyClass="stylePanel" id="portsSearchPanel">	
							    <f:facet name="header">		
							        <h:outputText value="Search Port"/>	
							    </f:facet>	
							    <rich:panel id="portsFilterPanel">	
							    	<table width="100%" cellpadding="0" cellspacing="0" border="0" style="border-collapse: separate;border-spacing:  4px 0px;" >
										<tr>
											...
											<td >
						                        <h:inputText value="#{portControl.filterPort.city.name }" id="cityText" styleClass="textBox" style="width:120px" 
						                        ...
						                        <rich:suggestionbox id="suggestionBoxId" for="cityText" 
			                                        ...
			                                        <a4j:support event="onselect" ajaxSingle="true" reRender="stateCombobox,countryCombobox" 
			                                        	...
			                                        	<f:setPropertyActionListener value="#{result.cityId }" target="#{portControl.filterPort.city.cityId}"/>
			                                        </a4j:support>
			                                    </rich:suggestionbox>
											</td>
											...
											<td >	
												<h:selectOneMenu id="stateCombobox" value="#{portControl.filterPort.city.stateProvince.valueId }" style="width:146px" onkeydown="if(event.keyCode == 13) search()" >
											...
											<td >										
												<h:selectOneMenu id="countryCombobox" value="#{portControl.filterPort.city.country.valueId }" style="width:123px" onkeydown="if(event.keyCode == 13) search()" >
												...
												<table>
													<tr>
														<td align="right" nowrap="nowrap">
															<a4j:commandLink action="#{portControl.searchPortsAction }" id="searchButton" reRender="portsDataTable, found" status="ajaxStatus">
													...
													<tr>
														<td align="center">
															...
															reRender="portsSearchPanel" ajaxSingle="true" status="none" immediate="true"/>
														</td>
													</tr>
												</table>
											</td>	
					...
					<tr>
						<td align="right">		
	                    <a4j:commandLink action="#{portControl.newPortAction}" value="" id="linkNew" 
					...
					<tr>
						<td>
							<rich:panel headerClass="stylePanel" bodyClass="stylePanel">	
							    ...
							    <rich:extendedDataTable id="portsDataTable" height="510px" rowClasses="row1, row2" rows="23"
									 value="#{portControl.ports }" var="port" reRender="ds"
									sortMode="#{portControl.sortMode }" selectionMode="#{portControl.selectionMode }"
									tableState="#{portControl.tableState }" binding="#{portControl.table }" title="Ports"> 
									<a4j:support
	       									event="onRowDblClick"
	       									action="#{portControl.selectPortAction}"  
	       									oncomplete="#{rich:component('editPanel')}.show()" 
	       									reRender="formPortModal"/>
									...
									<f:facet name="footer">
										<rich:datascroller align="center" for="portsDataTable" maxPages="30" id="ds" status="none"/>
									</f:facet>
			...
			<f:facet name="header">
		    		<h:outputText value="Port Details" />
	    	</f:facet>
			...
			<h:form id="formPortModal">
		  		<rich:messages style="color:red;"></rich:messages>
				<rich:hotKey key="esc" handler="#{rich:component('editPanel')}.hide();"/>
				<table width="98%" cellpadding="4">
					<tr height="50" valign="bottom">
						<td nowrap="nowrap" width="100" align="right">Port Name:</td>
						<td>
							<h:inputText id="nameText"
								value="#{portControl.port.name}" style="width:180px;" required="true" tabindex="1" requiredMessage="- Name field is required"/>
					...
					<tr>
						...
						<td >
						  <h:inputText value="#{portControl.port.city.name }" id="cityText" styleClass="textBox" style="width:120px" 
						      onkeydown="if (event.keyCode == 8){ clearCity() }" onkeyup="if(event.keyCode == 13) search()" tabindex="2"/>
						      <a4j:jsFunction name="clearCity" action="#{portControl.clearCity}" status="none" ajaxSingle="true" reRender="stateCombobox,countryCombobox"/>
							   		<rich:suggestionbox id="suggestionBoxId" for="cityText" 
			                	       suggestionAction="#{portControl.autocompleteCity}" var="result"
			                           tokens="," height="150" width="160" cellpadding="2"
					...
					<tr>
						...
						<td >	
							<h:selectOneMenu id="stateCombobox" value="#{portControl.port.city.stateProvince.valueId }" style="width:146px" onkeydown="if(event.keyCode == 13) search()"  disabled = "true" tabindex="3">
								<f:selectItem itemLabel="All" itemValue="0" />
								<f:selectItems value="#{portControl.provinces}"/>											
							</h:selectOneMenu>  	
						</td>
					...
					<tr>
						...
						<td >										
							<h:selectOneMenu id="countryCombobox" value="#{portControl.port.city.country.valueId }" style="width:123px" onkeydown="if(event.keyCode == 13) search()"  disabled = "true" tabindex="4">
								<f:selectItem itemLabel="All" itemValue="" />
								<f:selectItems value="#{portControl.countries}"/>											
						    </h:selectOneMenu> 
						</td>			
					...
					<tr>
						...
						<td>
							...
							value="#{portControl.port.iataCode}" style="width:80px;" tabindex="5"/>
						</td>
					</tr>
					<tr>
						<td colspan="3" align="center">
							<table cellspacing="17">
								<tr >				
									<td align="center">
										<a4j:commandButton value="Save" action="#{portControl.savePortAction}"
											type="button" styleClass="boton" tabindex="6" status="ajaxStatus" data="#{facesContext.maximumSeverity.ordinal ge 2}"
